...
cover: assets/projects/compass-ble/cover.webp
...
## Задача
Нужно было создать единое мобильное приложение для работы с Bluetooth Low Energy BMS-устройствами:

- быстрое и стабильное сканирование BLE-устройств;
- безопасное подключение/отключение и отображение статуса соединения;
- визуализация ключевой телеметрии батареи в реальном времени (ячейки, температура, ошибки, остаток заряда);
...
## Проект активно развивался итеративно. Ниже ключевые этапы разработки:

1. **Идентификация приложения**
   - изменение app id на `com.compass.stark`.

2. **Базовая структура и инфраструктура**
...
4. **Use case-ориентированный слой**
...
7. **Углубление протокольной поддержки**
...
8. **Расширение покрываемых устройств и UX**
   - добавление Daly BMS;
   - обновление иконок, splash screen;
   - фикс `scanDevices` для DL-T серии;
...
## Архитектура
![Схема](assets/projects/compass-ble/architecture.webp)

- Clean Architecture (с практичным адаптивным уклоном под BLE/IoT кейсы)
- Feature-first структура в presentation/data слоях
- UseCase-driven domain слой
- Репозитории + коннекторы + протокольный парсинг
- Платформенная интеграция через MethodChannel/EventChannel

### Структура модулей (укрупнённо)
- **Core**
...
- `jk_dual_char_connector.dart` использует native path на Android;
...
- Splash screen, дизайн-обновления, локализация/переводы экранов.
